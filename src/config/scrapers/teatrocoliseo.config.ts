...
export const teatroColiseoConfig: ScraperConfig = {
  ...
  listing: {
    ...
    // Contenedor principal: SOLO el PRIMER .vc_grid-container
    // (asume que "PROGRAMACIÓN ACTUAL" es siempre la primera sección)
    containerSelector: '.vc_grid-container:first',

    // Cada evento es un grid-item dentro del contenedor
 ...
 *    - Solución: usar `.vc_grid-container:first` (toma solo el primer grid)
 *    - ASUME que "PROGRAMACIÓN ACTUAL" es siempre la primera sección
 *    - Si el orden cambia, el selector debe ajustarse manualmente
 *    - No hay ID/clase única para diferenciar secciones en el HTML
 *
